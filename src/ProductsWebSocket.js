    ...
    static inicializar() {
        ...
        wsServer.on("request", (request) => {
            ...
            connection.on("message", async message => {
                ...
                // Nuevo en chat: Para envíar mensajes de chat primero transformo el string en objeto con json.parse
                // le agrego la fecha y lo vuelvo a mandar
                const objetoMensaje = JSON.parse(message.utf8Data);
                objetoMensaje.fecha = `${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}`;
                await PersistenciaChat.insertarMensaje({
                    email: objetoMensaje.email,
                    fecha: objetoMensaje.fecha,
